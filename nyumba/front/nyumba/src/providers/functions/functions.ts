...
export class FunctionsProvider {
  ...
  private persistMsg: Array<string> =  ['loginSuccess','logout','deleted','success'];
...
//Example: '/add','apt.id', '$event', 'Room','[user,apt,room]'
  ...
  filterData(queryText: string, apts: Apt[], searchFields: Array<string>, searcher:any):Apt[] {

    queryText = queryText.toLowerCase().replace(/,|\.|-/g, ' ');
    let queryWords = queryText.split(' ').filter(w => !!w.trim().length);

    return apts.map(a => searcher(a,queryWords,searchFields));
  }
